...
# 1. Embedding API
EMBEDDING_API_URL = "https://www.begm3.com/v1/embeddings" 
EMBEDDING_MODEL_NAME = "bge-m3"

# 2. Rerank API
RERANK_API_URL = "https://www.begreranker/v1/rerank" 
RERANK_MODEL_NAME = "bge-reranker-v2-m3"

# 3. DeepSeek API (Chat Completion)
# 注意：V3 和 R1 使用相同的 URL 端点，只是 model 参数不同
DEEPSEEK_API_URL = "https://www.deepseek.com/v1/chat/completions"
DEEPSEEK_R1_MODEL_NAME = "DeepSeek-R1"
DEEPSEEK_V3_MODEL_NAME = "DeepSeek-V3" # 新增 V3 模型用于 Query Rewrite

# ================= System Prompts =================

# --- 1. Query Rewrite Prompt (DeepSeek V3) ---
# 根据提供的 QueryRewritePrompt.txt 整合 System 和 Instruction
...
# --- 2. Final Answer Prompt (DeepSeek R1) ---
...
class RecallWorker(QThread):
    ...
    def __init__(self, query_text, db_path, json_path):
        super().__init__()
        self.original_query = query_text # 用户原始输入
        self.search_query = query_text   # 实际用于向量检索的 Query (可能是重写后的)
    ...
    def rewrite_query(self, original_query):
        ...
        payload = {
            ...
            "messages": messages,
            "temperature": 0.7, # 稍微增加创造性以进行扩写
            "stream": False     # 重写不需要流式
        }

        try:
            start_time = time.time()
            # 设置较短的超时时间，保证 RAG 整体响应速度
    ...
    def rerank_with_bge(self, query, candidates_text_list):
        ...
        payload = {
            ...
            "query": query, # 使用重写后的 Query 进行 Rerank 匹配度更高
    ...
    def call_deepseek_summary(self, user_original_query, top_results):
        # 注意：这里使用用户的 原始查询 (user_original_query) 传给 LLM
        # 因为我们希望 LLM 回答的是用户的问题，而不是回答机器扩写后的问题。
        # 扩写后的 Query 仅用于检索 (Retrieval & Rerank)
...
"""
        ...
        headers = { 'Content-Type': 'application/json', 'Authorization': f'Bearer {API_KEY}' }
        payload = {
            "model": DEEPSEEK_R1_MODEL_NAME, # 使用 R1 进行总结
            "messages": [
        ...
        try:
            ...
            if rerank_scores and len(rerank_scores) == len(processed_candidates):
                for idx, candidate in enumerate(processed_candidates):
                    ...
                    # 使用 search_query 进行规则裁决 (匹配技术词汇等)
            ...
            # 7. DeepSeek R1 Summary (使用 original_query)
            # 确保回答的是用户最初的问题，而不是扩写后的
            self.call_deepseek_summary(self.original_query, final_top_10)
...
class RAGRecallApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RAG 工业级全流程 (Recall -> Rewrite -> Rerank -> DeepSeek R1)")
        self.resize(1300, 950) 
        self.setStyleSheet(STYLESHEET)
        
        self.settings = QSettings("MyCorp", "RAGRecall_Final_v3")
        self.cached_results = []
        self.cached_summary = ""
        self.cached_query = ""
    ...
    def init_ui(self):
        ...
        # Search Button
        self.btn_search = QPushButton("🚀 执行全流程 (Rewrite + Recall + DeepSeek)")
